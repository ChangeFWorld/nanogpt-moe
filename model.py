...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.moe_enabled:
            self.mlp=SparseMoeBlock(embed_dim=config.n_embd, num_experts=config.num_experts, num_experts_per_tok=config.num_experts_per_tok, expert_hidden_dim=config.expert_hidden_dim, ffn_type=config.ffn_type)
        else:
            self.mlp = MLP(config)
...
class MoEGate(nn.Module):
    def __init__(self, embed_dim, num_experts=16, num_experts_per_tok=2, aux_loss_alpha=0.01):
        ...
        self.scoring_func = 'softmax' # or 'sigmoid'
...
class MoeMLP(nn.Module):
    def __init__(self, hidden_size, intermediate_size, act_layer=nn.SiLU):
        super().__init__()
...
class SparseMoeBlock(nn.Module):
    ...
    """
    def __init__(self, embed_dim, mlp_ratio=4, num_experts=16, num_experts_per_tok=2, expert_hidden_dim=512, ffn_type="glu", act_fn="gelu"):
        super().__init__()
        ...
        if act_fn == "gelu":
            ...
            raise NotImplementedError(f"act_fn {act_fn} is not implemented")
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states.repeat_interleave(self.num_experts_per_tok, dim=0) # (64*256*2, 384)
        y = torch.empty_like(hidden_states, dtype=hidden_states.dtype)
        for i, expert in enumerate(self.experts): 
            y[flat_topk_idx == i] = expert(hidden_states[flat_topk_idx == i]).to(y.dtype)
        y = (y.view(*topk_weight.shape, -1) * topk_weight.unsqueeze(-1)).sum(dim=1)
        y =  y.view(*orig_shape)
        y = AddAuxiliaryLoss.apply(y, aux_loss) if self.training else y
